import { getId } from '@/utils'
import {
  Collapse,
  Form,
  Input,
  Image as AntdImage,
  Switch,
  ImageProps as AntdIamgeProps,
} from 'antd'
import React, { useMemo } from 'react'
import { parserActions, propItemName } from '..'
import { NodeComponent } from '../../../type'
import AddAction from '../../add-action/inidex'
import { useEditorPreviewContext } from '../../editor-preview/provider'

const fallbackImagesrc =
  'data:image/png;base64,'

export interface ImageProps extends AntdIamgeProps {
  src: string
  alt?: string
  preview?: boolean
}

const Image: NodeComponent<ImageProps> = ({ node, editType }) => {
  const { props, style, actions: actionsProp } = node
  const { actions } = useEditorPreviewContext()
  const events = useMemo(
    () => parserActions(actionsProp || {}, actions, editType),
    [actions, actionsProp, editType]
  )
  return (
    <AntdImage
      fallback={fallbackImagesrc}
      style={style}
      {...props}
      {...events}
    />
  )
}

const ImagePropsForm: typeof Image['PropsForm'] = () => {
  return (
    <Collapse defaultActiveKey={['props', 'actions']} bordered={false}>
      <Collapse.Panel header="属性" key="props">
        <Form.Item label="图片地址" name={propItemName('src')}>
          <Input />
        </Form.Item>
        <Form.Item label="图片宽度" name={propItemName('width')}>
          <Input />
        </Form.Item>
        <Form.Item label="图片高度" name={propItemName('height')}>
          <Input />
        </Form.Item>
        <Form.Item label="图像描述" name={propItemName('alt')}>
          <Input />
        </Form.Item>
        <Form.Item
          label="预览功能"
          name={propItemName('preview')}
          valuePropName="checked"
        >
          <Switch />
        </Form.Item>
      </Collapse.Panel>
      <Collapse.Panel header="动作设置" key="actions">
        <Form.Item name="actions">
          <AddAction menus={[{ event: 'onClick', title: '点击图片' }]} />
        </Form.Item>
      </Collapse.Panel>
    </Collapse>
  )
}
Image.PropsForm = ImagePropsForm
Image.nodeName = 'image'
Image.title = '图片'
Image.getInitialStyle = () => ({
  display: 'inline-block',
})
Image.getInitialProps = () => ({
  preview: true,
  src: '',
})
Image.getId = () => getId('image')
export default Image
